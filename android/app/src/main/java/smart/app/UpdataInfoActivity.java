...
public class UpdataInfoActivity extends Activity implements View.OnClickListener {
    ...
    private void init() {
        ...
        accountinfo = (HashMap<String, Object>) getIntent().getSerializableExtra("accountinfo");
        try {
            lastname.setText(accountinfo.get("lastname").toString());
            firstname.setText(accountinfo.get("firstname").toString());
            nicktname.setText(accountinfo.get("nickname").toString());
            companyname.setText(accountinfo.get("companyname").toString());
            departmentname.setText(accountinfo.get("departmentname").toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
                    ...
                    public void run() {
                        try {
                            if (!TextUtils.isEmpty(firstname.getText().toString().trim()) && !TextUtils.isEmpty(lastname.getText().toString().trim()) && !TextUtils.isEmpty(nicktname.getText().toString().trim()) && !TextUtils.isEmpty(companyname.getText().toString().trim()) && !TextUtils.isEmpty(departmentname.getText().toString().trim())) {
                                hashMap = HttpService.authorizeaccess(firstname.getText().toString().trim(), lastname.getText().toString().trim(), nicktname.getText().toString().trim(), companyname.getText().toString().trim(), departmentname.getText().toString().trim());
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
    ...
    Runnable udpUIRunnable = new Runnable() {
        ...
        public void run() {
            ...
            if (!TextUtils.isEmpty(firstname.getText().toString().trim()) && !TextUtils.isEmpty(lastname.getText().toString().trim()) && !TextUtils.isEmpty(nicktname.getText().toString().trim()) && !TextUtils.isEmpty(companyname.getText().toString().trim()) && !TextUtils.isEmpty(departmentname.getText().toString().trim())) {
                Toast.makeText(UpdataInfoActivity.this, "修改成功", Toast.LENGTH_LONG).show();
            }
        }
    };
}